...
function animateScreenIn(screenId) {
  ...
  // Reset
  gsap.set(screen.querySelectorAll('.hero-text, .feature-card, .input-group, .btn, .thread, .dashboard-header'), {
    y: 30,
    opacity: 0
  });

  // Stagger In
  gsap.to(screen.querySelectorAll('.hero-text, .feature-card, .input-group, .btn, .thread, .dashboard-header'), {
    y: 0,
    opacity: 1,
    duration: 0.6,
    stagger: 0.05,
...
const chatObserver = new MutationObserver((mutations) => {
  mutations.forEach((mutation) => {
    if (mutation.target.id === 'chatPanel' && mutation.attributeName === 'class') {
      if (chatPanel.classList.contains('open')) {
        // Animate bubbles when chat opens
        gsap.fromTo('#messages .bubble', 
          { y: 20, opacity: 0 },
...
document.querySelectorAll('.btn, .feature-card, .thread').forEach(el => {
  el.addEventListener('mousedown', () => gsap.to(el, { scale: 0.96, duration: 0.1 }));
  el.addEventListener('mouseup', () => gsap.to(el, { scale: 1, duration: 0.1 }));
  el.addEventListener('mouseleave', () => gsap.to(el, { scale: 1, duration: 0.1 }));
});
