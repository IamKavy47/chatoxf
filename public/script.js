...
async function onRegister(){
  ...
  try {
    // request OTP from backend (Convex)
    const otp = await client.mutation("otp:requestOtp", {
      email,
      purpose: "register"
    });

    // attempt to send via mailer endpoint (best-effort)
    try {
      ...
      if(!j || !j.ok) console.warn("OTP mailer reported failure or no JSON:", j);
    } catch (e) {
  ...
  q("otpResendBtn").onclick = async ()=>{
    ...
    try {
      ...
      try {
        ...
        const j = await res.json().catch(()=>null);
        if(!j || !j.ok) console.warn("OTP mailer reported failure on resend:", j);
      } catch (e) {
